...
- [ ] **NAV-01**: A persistent bottom navigation bar replaces the current mobile nav on screens below `md`
- [ ] **NAV-02**: Mobile menu scroll-lock uses Headless UI Dialog (not `document.body.style.overflow`) to work on iOS
- [ ] **NAV-03**: All pages render correctly with both top-bar (desktop) and bottom-bar (mobile) navigation present
...
- [ ] **CAR-03**: `UpcomingCarousel` migrated to Embla with touch swipe and viewport-relative slide widths
- [ ] **CAR-04**: `WatchProviderCarousel` migrated to Embla with touch swipe
- [ ] **CAR-05**: `GenreCardList` migrated to Embla or CSS scroll-snap with touch swipe
- [ ] **CAR-06**: `TrendingCarousel` hero has touch swipe and uses `svh`/`dvh` for height
- [ ] **CAR-07**: No carousel card overflows a 375px viewport (hardcoded `w-lg`/`w-md` widths removed)
...
- [ ] **DETAIL-02**: Genre chips, rating badges, and action buttons wrap correctly on 375px without overflow
- [ ] **DETAIL-03**: Cast row is horizontally scrollable on mobile with touch swipe
...
- [ ] **SEC-02**: Genre detail page is responsive
- [ ] **SEC-03**: Collection page is responsive
- [ ] **SEC-04**: Provider and providers list pages are responsive
- [ ] **SEC-05**: Legal pages (Privacy Policy, Terms of Service) are readable on mobile
