...
const MediaSimilarCard = (p: MediaSimilarCardProps) => {
  const {
    ...
    release_date, first_air_date, overview, vote_average, vote_count,
  ...
  const initial = useMemo<Partial<DetailMedia>>(
    () => ({
      title, name, overview, vote_average, vote_count, genre_ids, original_language,
    ...
    [title, name, overview, vote_average, vote_count, genre_ids, original_language, poster_path, backdrop_path, release_date, first_air_date, runtime, number_of_seasons, number_of_episodes]
  );
        ...
        <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-sky-500/20 transition-all duration-200 bg-transparent h-full w-xs">
          ...
          <div className="flex-1 min-w-0 h-full flex flex-col justify-center">
            ...
            <p className="text-xs text-gray-400 mt-1 truncate">
              ...
              {media_type === "movie" && typeof runtime === "number" ? ` • ${Math.floor(runtime / 60)}h ${runtime % 60}min` : ""}
            </p>
          </div>
        </div>
      </Link>
